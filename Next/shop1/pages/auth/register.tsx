...
const register = () => {
    ...
    const [registerInput, setRegisterInput] = useState<RegisterInputs>({
        email: "",
        username: "",
        password: "",
    })
    ...
    const handleSubmit = async (event:FormEvent<HTMLFormElement>) => {
        ...
        if (registerInput.password.length < 6) {
            setPasswordError("Password too short, need at least 6 characters")
        } else {
            setPasswordError("")
        }
        ...
        try {
            ...
            const response = axios.post("/api/auth/standard", {
                type: "REGISTER",
                    ...
                    className="bg-emerald-600 h-full w-full p-4 flex flex-col gap-5 justify-center">
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex gap-5">
                                ...
                                <button type="button" onClick={() => setShowPass(!showPass)} className="w-fit outline-none
                                p-2 transition-colors duration-150 bg-emerald-100 border-4 border-emerald-200 hover:border-emerald-800">
                        ...
                        <div className="flex gap-5">
                            <button type="submit" className="w-fit p-2 bg-emerald-100 border-4 border-emerald-200 
                            hover:border-emerald-800 transition-colors duration-150">Register</button>
                            <Link className="w-fit p-2 bg-emerald-100 border-4 border-emerald-200 
                            hover:border-emerald-800 transition-colors duration-150"
                            href={"/auth/login"}><span className="md:inline hidden">Already registered?</span> Login here!</Link>
                        </div>
                    </form>                    
                </div>
            </div>
